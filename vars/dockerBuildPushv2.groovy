...
def call(Map params = [:]) {
  ...
  try {
    container('docker') {
      sh "rm -rf srcrepo || true"
      if (gitCredsId) {
        echo "[dockerBuildPush] Private repo kullanılıyor"
        withCredentials([usernamePassword(credentialsId: gitCredsId, usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
          sh """
            set -euxo pipefail
            git clone -b '${gitBranch}' https://\$GIT_USER:\$GIT_PASS@${gitRepo.replace('https://','')} srcrepo
          """
        }
      } else {
        echo "[dockerBuildPush] Public repo kullanılıyor"
        sh """
          set -euxo pipefail
